...
# ╔═╡ bf3615b1-3a51-4ac3-bfe1-47f56518a4f3
begin
	using CSV
	using DataFrames
	using Downloads
...
# ╔═╡ cd60cf5b-1e5d-4e3d-8171-ba87b50ad71d
using Pipe: @pipe

# ╔═╡ 62e8b182-417e-11ec-1b21-ab0009235240
md"""
# Data Dojo 6 - Lemurs
...
Today we are using `julia` instead of `R` and we'll try to answer a couple of questions:
...
# ╔═╡ 9511af04-6f40-44ba-a253-5d439d7be550
md"""
## Loading the Data
"""

# ╔═╡ 38427d47-3e1c-4088-b835-d95327a8c526
lemurs = CSV.File(Downloads.download("https://raw.githubusercontent.com/rfordatascience/tidytuesday/master/data/2021/2021-08-24/lemur_data.csv")) |> DataFrame

# ╔═╡ 2c3491af-87ad-4bcb-b2f3-df246c761b3f
summary(lemurs)

# ╔═╡ 27a58762-55c9-429f-888b-86347e4df570
describe(lemurs)

# ╔═╡ 9b50510e-501c-4ba4-b434-e66b330fe988
md"""
## 1. Which lemur had the most offspring?

There is a column `n_known_offspring`, so we'll select the lemur (Id and Name) and that column, make entries unique and sort by it:
"""

# ╔═╡ b541f6f8-6a9b-4092-bec3-ef1cb2f220e1
to_number(x) = parse.(Int, x)

# ╔═╡ 21feee31-c860-4142-8d79-d96652041b54
	lemurs_with_offspring = @pipe lemurs |>
		select(_, :dlc_id, :name, :sex, :n_known_offspring) |>
		unique |>
		filter(:n_known_offspring => !=("NA"), _) |>
		transform(_, :n_known_offspring => (x -> parse.(Int,x)) => :n_known_offspring) |>
		sort(_, :n_known_offspring, rev=true)
